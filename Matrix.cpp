...
matrix& matrix::dowroc() {
    ...
    for (int i = 0;i < n;i++) for (int j = 0;j < n;j++) tmp[j * n + i] = data[i * n + j];
    data = std::move(tmp);
    return *this;
}
matrix& matrix::losuj() {
    for (int i = 0;i < n * n;i++) data[i] = std::rand() % 10;
    return *this;
}
matrix& matrix::losuj(int x) {
    for (int i = 0;i < x;i++) data[std::rand() % (n * n)] = std::rand() % 10;
    return *this;
}
...
matrix& matrix::diagonalna(int* t) {
    ...
    for (int i = 0;i < n;i++) data[i * n + i] = t[i];
    return *this;
}
...
matrix& matrix::wiersz(int y, int* t) {
    for (int j = 0;j < n;j++) data[y * n + j] = t[j];
    return *this;
}
...
matrix& matrix::pod_przekatna() {
    std::fill(data.get(), data.get() + n * n, 0);
    for (int i = 0;i < n;i++) for (int j = 0;j <= i;j++) data[i * n + j] = 1;
    return *this;
}
matrix& matrix::nad_przekatna() {
    std::fill(data.get(), data.get() + n * n, 0);
    for (int i = 0;i < n;i++) for (int j = i + 1;j < n;j++) data[i * n + j] = 1;
    return *this;
}

// ===== 4.4 Szachownica =====
matrix& matrix::szachownica() {
    for (int i = 0;i < n;i++) for (int j = 0;j < n;j++) data[i * n + j] = (i + j) % 2;
    return *this;
}

// ===== 4.5 Operatory =====
matrix& matrix::operator+(matrix& m) { for (int i = 0;i < n * n;i++) data[i] += m.data[i]; return *this; }
...
// ===== 4.7 Operatory porównań =====
bool matrix::operator==(const matrix& m) const {
    if (n != m.n) return false;
    for (int i = 0;i < n * n;i++) if (data[i] != m.data[i]) return false;
    return true;
}
bool matrix::operator>(const matrix& m) const {
    int count = 0;
    for (int i = 0;i < n * n;i++) if (data[i] > m.data[i]) count++;
    return count >= (n * n / 2);
}
bool matrix::operator<(const matrix& m) const {
    int count = 0;
...
std::ostream& operator<<(std::ostream& o, const matrix& m) {
    for (int i = 0;i < m.n;i++) {
        for (int j = 0;j < m.n;j++) o << m.data[i * m.n + j] << " ";
        o << "\n";
    }
    return o;
}
